...
class MenuViewController: UITableViewController {
        ...
        let generalTitles = ["Home", "Profile"]
        let cateoriesTitle = ["Top Rated", "Upcoming", "Popular"]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (indexPath.section == 0 && indexPath.row == 0) {
            let homeViewController = self.storyboard?.instantiateViewController(withIdentifier: "MainTableViewController")
            navigationController.viewControllers = [homeViewController!]
        } else {
            let popularViewController = self.storyboard?.instantiateViewController(withIdentifier: "PopularViewController")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            let titles = generalTitles
            cell?.textLabel?.text = titles[indexPath.row]
        } else {
            let titles = cateoriesTitle
            cell?.textLabel?.text = titles[indexPath.row]
        }
